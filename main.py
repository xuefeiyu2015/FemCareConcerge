...
def build_root_agent():
    ...
    cycle_expert = Agent(
        ...
        instruction=(
            ...
            "onboarding). If they don't give a date, ask for it (or confirm 'today').\n"
            "CONFIRM BEFORE SAVING: Whenever the user mentions a period event — even casually, "
            "like 'I started my period today' — treat it as a possible log, but NEVER save it "
            "silently. First resolve the date (turn 'today' into a concrete YYYY-MM-DD, or ask "
            "if it's unclear), then recap what you would save and ask a clear yes/no question "
            "such as 'Would you like me to record that?'. Only call add_period_record with "
            "confirmed=True AFTER the user explicitly says yes. If you call it without "
            "confirmed=True, the tool returns {'status': 'needs_confirmation', ...}; relay that "
            "message and wait for the user's answer. If they decline, acknowledge warmly and "
            "save nothing. After a successful save, warmly confirm what you recorded, and if the "
            "tool result includes a 'note' field, gently share that note too.\n"
            ...
            "Once you have all three, recap the values and ask the user to confirm, then save "
            "them with add_period_record(start_date=..., end_date=..., cycle_length=..., "
            "confirmed=True) and warmly confirm what you recorded (relaying any 'note' field)."
        ),
